...
{
    public partial class frmSplitVideo : Form
    {
        public frmSplitVideo()
        {
            InitializeComponent();
        }
        ...
        private void cutVideo_Click(object sender, EventArgs e)
        {
            ...
            string command = $"-i \"{txtInputfile.Text}\" -ss {start} -t {end} -c copy \"{outputFileName}\"";

            ffmpeghandler.openFFMPEG(command);
        }

        private void splitVideo_Click(object sender, EventArgs e)
        {
            int end = Convert.ToInt16(endTextBox.Text);
            string outputFileName = $@"{txtOutputFolder.Text}\{txtNameFileOutput.Text}{extensionComboBox.Text}";
            string command = $"-i \"{txtInputfile.Text}\" -c copy -f segment -segment_time {end} -reset_timestamps 1 \"{outputFileName}\"";
